...
{
	LOG(L_TERM, 2, "MULTIX/terminal (log:%i, phy:%i): configure physical line", unit->log_num, unit->phy_num);
	if (unit && cfg_phy) {
...
{
	LOG(L_TERM, 2, "MULTIX/terminal (log:%i, phy:%i): configure logical line", unit->log_num, unit->phy_num);
	return E_OK;
}
...
{
	LOG(L_TERM, 2, "MULTIX/terminal (log:%i, phy:%i): attach", unit->log_num, unit->phy_num);
	unit->attached = 1;
...
{
	LOG(L_TERM, 2, "MULTIX/terminal (log:%i, phy:%i): detach", unit->log_num, unit->phy_num);
...
{
	LOG(L_TERM, 2, "MULTIX/terminal (log:%i, phy:%i): status", unit->log_num, unit->phy_num);
	return 0;
}
