...
void op_cw()
{
	alu_16_compare((int16_t) regs[IR_A], N);
}
...
void op_cl()
{
	alu_16_compare(regs[IR_A], (uint16_t) N);
}
...
void op_cb()
{
	uint8_t data;
	if (!mem_get_byte(NB, N, &data)) return;
	alu_16_compare((uint8_t) regs[IR_A], data);
}
