...
{
	EMULOG(L_PNRD, 10, "MULTIX/punchreader (line:%i): configure physical line", unit->phy_num);
	return E_OK;
}
...
{
	EMULOG(L_PNRD, 10, "MULTIX/punchreader (line:%i): configure logical line", unit->phy_num);
	return E_OK;
}
...
{
	EMULOG(L_PNRD, 10, "MULTIX/punchreader (line:%i): attach", unit->log_num);
	unit->attached = 1;
...
{
	EMULOG(L_PNRD, 10, "MULTIX/punchreader (line:%i): detach", unit->log_num);
	unit->attached = 0;
...
{
	EMULOG(L_PNRD, 10, "MULTIX/punchreader (line:%i): status", unit->log_num);
	return 0;
}
...
void mx_punchreader_cmd_transmit(struct mx_unit_proto_t *unit, uint16_t addr)
{
    ...
    EMULOG(L_PNRD, 1, "MULTIX/punchreader (line:%i): transmit", unit->log_num);
    int ret = E_OK;
