...
static void * uzfx_worker_loop(void *ptr)
{
	...
	while (!quit) {
		...
		switch (state) {
			case UZFX_STX_QUIT:
				LOG(L_UZFX, "Worker processing state: QUIT");
				quit = true;
				break;
			case UZFX_ST1_SECT_RD:
				LOG(L_UZFX, "Worker processing state: SECTOR READ");
...
static int uzfx_cmd_read(cchar_unit_t *unit, uint16_t *r_arg)
{
	...
	switch (uzfx->state) {
		case UZFX_ST0_IDLE:
			LOG(L_UZFX, "command: read (state: idle)");
		...
		case UZFX_ST2_BUF_RD:
			...
			*r_arg = c;
			...
			break;
		default:
			LOG(L_UZFX, "command: read (state: other)");
			io_ret = IO_EN;
			break;
	}
	pthread_mutex_unlock(&uzfx->state_mutex);
...
static int uzfx_cmd_write(cchar_unit_t *unit, const uint16_t *r_arg)
{
	...
	switch (uzfx->state) {
		case UZFX_ST0_IDLE:
			LOG(L_UZFX, "command: write (state: idle)");
			uzfx->state = UZFX_ST3_BUF_WR_START;
			pthread_cond_signal(&uzfx->state_cond);
			io_ret = IO_EN;
			break;
		...
		case UZFX_ST7_SECT_WR:
			...
			break;
		default:
			LOG(L_UZFX, "command: write (state: other)");
			io_ret = IO_EN;
			break;
	}
	pthread_mutex_unlock(&uzfx->state_mutex);
...
static int uzfx_cmd_control(cchar_unit_t *unit, const uint16_t *r_arg, int cmd)
{
	...
	switch (uzfx->state) {
		case UZFX_ST0_IDLE:
			LOG(L_UZFX, "command: control (state: idle)");
			uzfx->operation = cmd;
...
static int uzfx_cmd_detach(cchar_unit_t *unit)
{
	...
	switch (uzfx->state) {
		case UZFX_ST0_IDLE:
			LOG(L_UZFX, "command: detach (state: idle)");
			uzfx->interrupts = UZFX_INT_NONE;
			io_ret = IO_OK;
			break;
		case UZFX_ST2_BUF_RD:
			LOG(L_UZFX, "command: detach (state: buffer read)");
