...
{
	EMULOG(L_FLOP, 10, "MULTIX/floppy (line:%i): configure physical line", unit->phy_num);
	if (unit && cfg_phy) {
...
{
	EMULOG(L_FLOP, 10, "MULTIX/floppy (line:%i): configure logical line", unit->phy_num);
...
{
	EMULOG(L_FLOP, 10, "MULTIX/floppy (line:%i): attach", unit->log_num);
	unit->attached = 1;
...
{
	EMULOG(L_FLOP, 10, "MULTIX/floppy (line:%i): detach", unit->log_num);
	unit->attached = 0;
...
{
	EMULOG(L_FLOP, 10, "MULTIX/floppy (line:%i): status", unit->log_num);
	return 0;
}
...
{
    EMULOG(L_FLOP, 1, "MULTIX/floppy (line:%i): transmit", unit->log_num);
    int ret = E_OK;
